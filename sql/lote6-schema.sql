...
-- Seed: UYU -> USD (JP lo edita desde Ajustes)
...
CREATE TABLE IF NOT EXISTS costos_directos_proyecto (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  proyecto_id TEXT NOT NULL REFERENCES proyectos_cache(id) ON DELETE CASCADE,
  ...
  tc_aplicado NUMERIC(10, 4),  -- NULL si moneda_original=USD; si no, snapshot
  fecha DATE NOT NULL,
